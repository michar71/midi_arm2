...
public:
    ...
    void soft_gyro_calibration(const bool b)
    {
        soft_gyro_cal = b;
    }
    ...
    void setGyroBias(const float x, const float y, const float z) {
        ...
        //Only write cal to registers if we are not doing the software cal
        if (soft_gyro_cal == false)
        write_gyro_offset();
    }
...
private:
    void read_accel_gyro(int16_t* destination) {
        ...
        //Subtract soft gyro cal
        if (soft_gyro_cal == true)
        {
            //Set Gyero Offsets to 0.
...
private:
    bool read_mag(int16_t* destination) {
        ...
        {    
            ...
            if (MAG_MODE == 0x02 || MAG_MODE == 0x04 || MAG_MODE == 0x06)    // continuous or external trigger read mode
            {
                ...
                {
                    //Try to clear DOR Error by reading ST2
                    st1 = read_byte(AK8963_ADDRESS, AK8963_ST2);
                    Serial.print("MAG st2:");
                    Serial.println(st1);
    ...
    void collect_soft_gyro_cal()
    {
        ...
        //Set Hardware compensation registers to 0
        write_gyro_offset();
        delay(20);

        for (int ii=0;ii<CAL_SAMPLES;ii++)
        {
            read_accel_gyro(raw_acc_gyro_data);
            gbx += raw_acc_gyro_data[4];
            gby += raw_acc_gyro_data[5];
            gbz += raw_acc_gyro_data[6];
            delay(30);
        }

        gyro_bias[0] = gbx / CAL_SAMPLES;
        gyro_bias[1] = gby / CAL_SAMPLES;
        gyro_bias[2] = gbz / CAL_SAMPLES;

          if (b_verbose) 
        {
            Serial.println("Soft Gyro Calibration done!");

            Serial.println("Gyro biases (deg/sec)");
    ...
    {
        //Collect software gyro cal while the gyro is configured in normal operating mode
        if (soft_gyro_cal == true)
        {
            //Collect calibration data
            collect_soft_gyro_cal();
        }
        ...
        //Do not set the hardware registers if its soft gyro cal.
        if (soft_gyro_cal == false)
            write_gyro_offset();
        delay(100);
        initMPU9250();
    ...
    void write_gyro_offset() {
       ...
       {
            uint8_t data[6];  
            read_bytes(mpu_i2c_addr, XG_OFFSET_H, 6, &data[0]);
            for(int ii=0;ii<6;ii++)
            {
                Serial.print("GYRO Cal Readback");
                Serial.print(ii);
                Serial.print(":");
