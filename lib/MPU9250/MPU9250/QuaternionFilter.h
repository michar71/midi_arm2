...
class QuaternionFilter {
    // for madgwick
    float GyroMeasError = PI * (30.0f / 180.0f);     // gyroscope measurement error in rads/s (start at 40 deg/s)
    ...
    void update(float ax, float ay, float az, float gx, float gy, float gz, float mx, float my, float mz, float* q) {
        ...
        switch (filter_sel) {
            ...
            case QuatFilterSel::MAHONY:
                ...
                break;   
            case QuatFilterSel::MAHONY_EX:
                 MahonyQuaternionUpdate(ax, ay, az, gx, gy, gz, mx, my, mz,q);
                break;           
            default:
                no_filter(gx, gy, gz, q);
                break;
        }
    }

    void no_filter(float gx, float gy, float gz, float* q) {
