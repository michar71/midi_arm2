  ...
  if (ignorelines == 0)
  {
    ...
    else if (list[0].contains("C"))
    {
      ...
      else
      {
        ...
        //Filterring messes with the transition.
        //We need to reinit the filter when that happens
        if (abs(p - KalFilterP.getKalmanGain()) > 1)
          p = KalFilterP.setValue(p);
        else
          p = KalFilterP.getFilteredValue(p);
        
        if (abs(r - KalFilterR.getKalmanGain()) > 1)
          r = KalFilterR.setValue(r);
        else
          r = KalFilterR.getFilteredValue(r);
        
        if (abs(y - KalFilterY.getKalmanGain()) > 1)
          y = KalFilterY.setValue(y);
        else
