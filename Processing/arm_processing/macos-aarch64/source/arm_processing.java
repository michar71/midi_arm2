/* autogenerated by Processing revision 1293 on 2023-11-04 */
...
public float fourBytesToFloat(byte b1, byte b2, byte b3, byte b4)
{
...
public int limit(int in, int min, int max)
{
  if (in > max)
    return max;
  if (in < min)
...
public void calc_call_min_max()
{
  ...
  if (((minp <+ -3.1f) || (maxp >= 3.1f)) && (crossp == false))
  {
    crossp = true;
    minp = 65535;
    maxp = -65535;  
  }  
  if (((minr <= -3.1f) || (maxr >= 3.1f)) && (crossr == false))
  {
    crossr = true;
    minr = 65535;
    maxr = -65535;  
  }
  if (((miny <= -3.1f) || (maxy >+ 3.1f)) && (crossy == false))
  {
    crossy = true;
    miny = 65535;
    maxy = -65535;  
...
public void process_received_string(String myString)
{
  ...
  else if (list[0].contains(String.valueOf(ID_DATA)))
  {
    ...
    else
    {
      ...
      if (tension_ch1 >= 0)
        tension_ch1 = PApplet.parseInt(map(tension_ch1,0,255,255,0));
      if (tension_ch2 >= 0)  
        tension_ch2 = PApplet.parseInt(map(tension_ch2,0,255,255,0));
...
class kalman {
  ...
  public void settings() { size(400, 260, P3D);
smooth(); }
